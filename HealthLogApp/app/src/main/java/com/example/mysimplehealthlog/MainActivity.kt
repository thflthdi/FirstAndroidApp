...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        transaction.replace(binding.mainFrame.id, HomeFragment()).commit()
        ...
        tabLayout.addOnTabSelectedListener(object : TabLayout.OnTabSelectedListener {
            override fun onTabReselected(tab: TabLayout.Tab?) {
                ...
                when (tab?.position) {
                    0 -> transaction.replace(binding.mainFrame.id, HomeFragment())
                    1 -> transaction.replace(binding.mainFrame.id, AnalysisFragment())
                    2 -> transaction.replace(binding.mainFrame.id, PlanFragment())
                    3 -> transaction.replace(binding.mainFrame.id, PlusFragment())
                }
                transaction.commit()
            }
